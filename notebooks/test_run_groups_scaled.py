...
def main(class_count):
    ...
    model.evaluate(model, class_count, class_type.Group, 1, "Raw  - Scaled", train_type.default,gen_confusion_matrix=True)
    model.train_distillation_yolo(split_type=split_type.KFOLD, class_type=class_type.Group,
                                  dataset_type=dataset_type.SCALED,epochs=model.num_destillation_epochs, k_fold_value=1,
                                  classes_count=class_count, use_soft_distillation=True)
    model.evaluate(model, class_count=class_count, class_type=class_type.Group, k_fold_value=1, name="Distilled Model - Scaled",
