...
def main(class_count):
    model = model_trainer()
    model.train_default_yolo(split_type.KFOLD, class_type.default, dataset_type.SCALED, epochs=model.num_yolo_epochs,
                             k_fold_value=5, classes_count=class_count)
    model.evaluate(model, class_count, class_type.default, 5, "Raw Yolo", train_type.default)
    ...
    model.evaluate(model, class_count=class_count, class_type=class_type.default, k_fold_value=5, name="Distilled Model - Scaled",
